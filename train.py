...
class DivCycleGAN():
    ...
    def diversity_loss(self, inputA, inputB, mask):
        ...
        if maskpixel > 0:
            ...
            return F.relu(self.Lambda - self.lossL1(regularized, self.zerocomparer) * totalpixel / maskpixel)
...
class Preprocesser():
    ...
    def __init__(self):
        ...
        # training time data argumentation
        parser.add_argument("--rescalesize", type=int, default=286)
        ...
        # GPU selections, only one GPU is allowed
...
if __name__ == '__main__':
    ...
    totalStep = (args.currentepoch - 1) * datasetSize
    for i in range(args.currentepoch, args.epoch + 1):
        print('Start epoch %d / %d' % (i, args.epoch))
        for j, data in enumerate(loader):
            ...
            totalStep += args.batchsize
            epochIter = totalStep - datasetSize * (i - 1)

            if totalStep % args.displayfrequency == 0:
                visualizer.display_current_results(model.get_current_visuals(), i)

            if totalStep % args.printfrequency == 0:
                errors = model.get_current_errors()
                visualizer.print_current_errors(i, epochIter, errors)
                visualizer.plot_current_errors(i, float(epochIter) / datasetSize, errors)

            if totalStep % args.savefrequency == 0:
                print("saving the latest model (epoch %d)" % i)
                model.save(i)

        if i > args.decayepoch:
            model.decay()
